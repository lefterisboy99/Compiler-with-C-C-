...
%type <unsdValue> Q
%type <stmtValue> stmt
%type <stmtValue> stmts
%type <stmtValue> program
%type <stmtValue> ifstmt
%type <stmtValue> whilestmt
%type <stmtValue> forstmt
%type <stmtValue> block
%type <stmtValue> funcdef
...
stmt:       expr SEMICOLON
            ...
            |BREAK
            {
                ...
                $BREAK = new struct stmt_t;
                make_stmt($BREAK);
                
                emit(jump,nullptr, nullptr, nullptr, 0, yylineno);
                $BREAK->breakList = newlist(nextquadlabel() - 2);
            } 
            SEMICOLON
            { 
                if(loop_intersect){
            ...
            |CONTINUE
            {
                ...
                $CONTINUE = new struct stmt_t;
                make_stmt($CONTINUE);
                
                emit(jump,nullptr, nullptr, nullptr, 0, yylineno);
                $CONTINUE->contList = newlist(nextquadlabel() - 2);
            } 
            SEMICOLON 
            { 
                cout << "program -> stmt*" << endl;
            ...
            |funcdef    { cout << "stmt -> funcdef" << endl;$$=$1; }
            |SEMICOLON  { cout << "stmt -> ;" << endl;$$=NULL; }
            ;
...
expr:       assignexpr  
            ...
            { 
                cout << "expr -> expr && expr" << endl; 
                $$ = new_expr(boolexpr_e);
                $$->sym = assign_temp($1,$4, SCOPE, yylineno, FUNC_DEPTH);

                if($1->type == constbool_e && $4->type == constbool_e){
                    $$->type = constbool_e;
                }

                if ($4->falselist.empty()) {
                    $4->falselist = backup_falselists.back();
                    $4->truelist = backup_truelists.back();
                    backup_falselists.pop_back();
                    backup_truelists.pop_back();
                    isrelop = true;
                }

                if ($1->falselist.empty()) {
                    $1->falselist = backup_falselists.back();
                    $1->truelist = backup_truelists.back();
                    backup_falselists.pop_back();
                    backup_truelists.pop_back();
                    isrelop = true;
                }
                ...
                for(int i=0; i<$4->falselist.size(); i++){
                    $1->falselist.push_back($4->falselist.at(i));
                }
            ...
            { 
                ...
                for(int i=0; i<$4->truelist.size(); i++){
                    $1->truelist.push_back($4->truelist.at(i));
                }
                $$->truelist = $1->truelist;
...
term:       START_PARENTHESIS expr END_PARENTHESIS  
            ...
            |NOT expr   
            {
                ...
                $2 = new_expr(boolexpr_e);
                isrelop = true;
                $2->truelist.push_back(nextquadlabel());
                emit(if_eq, $2 , nullptr, nullptr, nextquadlabel() + 2, yylineno);
...
primary:    lvalue  {
            ...
            |const  { cout << "primary -> const" << endl;$$=$1; }
            ;
