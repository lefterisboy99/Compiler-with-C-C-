...
expr:       assignexpr  
            ...
            |expr EQUAL{
                if(isnt){
                    ...
                    e->boolConst=1;
                    emit(assign, e, nullptr, $1, 0, yylineno);
            ...
            } expr
            {
                ...
                $$->truelist.push_back(nextquadlabel());
                emit(if_eq, $1 , $4, nullptr, nextquadlabel() + 2, yylineno);
            ...
            { 
                ...
                cout<<"sizeee"<<$1->truelist.size()<<$Q<<endl;
                backpatch($1->truelist, $Q);
                
                for(int i=0; i<$1->falselist.size(); i++){
                    cout<<"heeey"<<$1->falselist.at(i)<<endl;
                }
                for(int i=0; i<$1->falselist.size(); i++){
                ...
                if ($2->truelist.empty()) {
                  //  isLogic = false;
                   ...
                   // emit(assign, e, nullptr, $expr, 0, yylineno);
                    ...
                    e->boolConst = 0;
                    //emit(assign, e, nullptr, $expr, 0, yylineno);
                    backpatch($term->truelist, nextquadlabel() - 3);
                    backpatch($term->falselist, nextquadlabel() - 1);
                }  else {
                    $term->falselist = $expr->truelist;
                    $term->truelist =  $expr->falselist;
                }              
            ...
            ASSIGN expr
            {
                ...
                if($lvalue->type != tableitem_e)
                {
                    ...
                    if(!$expr->truelist.empty()) {
                        ...
                        backpatch($expr->falselist, nextquadlabel() - 1);
                        isLogic=false;
                        emit(assign,$lvalue, nullptr, nullptr, 0, yylineno);
                        emit(assign,$lvalue, nullptr, nullptr, 0, yylineno);
...
forprefix:          FOR {loop_counter++;}START_PARENTHESIS elist SEMICOLON M expr SEMICOLON
                    {
                        ...
                        if(isLogic) {
                            ...
                            emit(assign, e, nullptr, $expr, 0, yylineno);
                            cout<<"hey5"<<endl;
                            for(int i=0;i<$expr->truelist.size();i++)
                                cout<<$expr->truelist.at(i)<<endl;
                            cout<<endl;
                            for(int i=0;i<$expr->falselist.size();i++)
                                cout<<$expr->falselist.at(i)<<endl;
                            backpatch($expr->truelist, nextquadlabel() -3);
                            backpatch($expr->falselist, nextquadlabel() - 1);
