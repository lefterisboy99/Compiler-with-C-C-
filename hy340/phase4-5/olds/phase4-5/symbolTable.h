...
expr* make_call (expr* lv, vector <expr*> reversed_elist, int scope, unsigned int line, unsigned int func_depth) {
    ...
    while (!reversed_elist.empty()) {
        ...
        cout<<"reversed elist "<<reversed_elist.back()->type<<endl;
...
unsigned userfunc_newused (SymbolTableEntry* sym) {
    ...
    cout<<sym->value.funVal->name<<endl;
    cout<<sym->taddress<<endl;
    for (int i=0; i<userfunc_table.size(); i++) {   
        cout<<sym->value.funVal->name <<" "<< userfunc_table.at(i).id <<" "<<endl;
        if (sym->value.funVal->name == userfunc_table.at(i).id && userfunc_table.at(i).scope==sym->value.funVal->scope&&userfunc_table.at(i).locals==sym->value.funVal->totalLocals){
...
void generate (vmopcode op, quad* q)
{
    ...
    t.opcode = op;
    /*if(op==assign_v){
        cout<<currprocessedquad<<endl;
        if(q!=nullptr&&q->arg1!=nullptr&&q->arg2!=nullptr){
            cout<<"I am in1"<<endl;
            cout<<q->arg1->type<<endl;
            if(q->arg1->type==nil_e){
                cout<<"I am in2"<<endl;
                q->arg1->type=q->arg2->type;
            }
        }
    }*/
    make_operand(q->arg1, &t.arg1);
    cout<<"out of arg1"<<endl;
    make_operand(q->arg2, &t.arg2);
    cout<<"out of arg2"<<endl;
    make_operand(q->result, &t.result);
    cout<<"out of arg3"<<endl;
...
void generate_ASSIGN (quad* q)           { cout<<q->arg1->type<<endl<<endl<<endl;generate(assign_v, q); }
