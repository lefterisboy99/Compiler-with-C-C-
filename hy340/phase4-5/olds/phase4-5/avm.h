...
avm_memcell* avm_tablesetelem (avm_table* table, avm_memcell* index, avm_memcell* content) {
    ...
    switch (index->type)
    {
        case string_m :
        {
            ...
            //cout<<"set"<<i<<endl;
            bool f = false;
            //cout<<index->data.stringVal<<endl;
...
avm_memcell* avm_tablegetelem (avm_table* table, avm_memcell* index) {
    ...
    switch (index->type)
    {
        case string_m :
        {
            ...
            //cout<<"get"<<index->data.stringVal;
            //cout<<"get"<<i<<endl;
            while (tmp) {
               
                //cout<<tmp->key.data.stringVal<<" "<<index->data.stringVal<<endl;
                if (!strcmp(tmp->key.data.stringVal,index->data.stringVal)){
                    return &tmp->value;
                }
...
string table_tostring (avm_memcell* m) {
    ...
    for (int i=0; i<AVM_TABLE_HASHSIZE; i++) {
        ...
        while(temp) {
            cout << "{ " << temp->key.data.numVal << " : ";

            switch (temp->value.type)
            {
                case bool_m :   cout << bool_tostring(&temp->value) << " }";
                break;

                case userfunc_m :   cout << userfunc_tostring(&temp->value) << " }";
                break;

                case libfunc_m :   cout << temp->value.data.libfuncVal << " }";
                break;

                case number_m :   cout << temp->value.data.numVal << " }";
                break;

                case string_m :   cout << "\"" << temp->value.data.stringVal << "\"" << " }";
                break;

                case table_m :  table_tostring(&temp->value);
                break;

                default : assert(0);
            }
            counter++;
            if (counter < table->total) cout << ",";
            else cout << " ]";
            temp = temp->next;
        }
...
void libfunc_objectmemberkeys(void) {
    ...
    for (int i=0; i<(s_v.size() + d_v.size()); i++) {
        ...
        avm_tablesetelem(new_table, index, content);
