...
map <string, unsigned> funcenter_jump;
...
unsigned userfunc_newused (SymbolTableEntry* sym) {
    for (int i=0; i<userfunc_table.size(); i++) {
        ...
        if (sym->value.funVal->name == userfunc_table.at(i).id){
...
void generate_CALL (quad* q) {
   ...
   /* map <string, unsigned> :: iterator it = funcenter_jump.begin();
    it = funcenter_jump.find(q->result->sym->value.varVal->name);
    if (it != funcenter_jump.end()){
       // t.result.val = funcenter_jump.at(q->result->sym->value.varVal->name);
    }
    if(q->result->istemp){
        q->result->type=q->result->next->type;
        cout<<q->result->next->type<<endl;
        cout<<q->result->type<<endl;
        cout<<q->result->next->sym->value.varVal->name<<endl;
        it = funcenter_jump.find(q->result->next->sym->value.varVal->name);
        if (it != funcenter_jump.end()){
           // t.result.val = funcenter_jump.at(q->result->next->sym->value.varVal->name);
        }
    }*/
...
void generate_GETRETVAL (quad* q) {
    q->taddress = nextinstructionlabel();
    //cout<<"ret val "<<q->result->type<<endl;
    instruction t;
    t.opcode = assign_v;
    make_operand(q->result, &t.result);
...
void generate_FUNCSTART (quad* q) {
    ...
    fsym->taddress=nextinstructionlabel()+1;
    ...
    f.id = q->result->sym->value.funVal->name;
    f.locals = q->result->sym->value.funVal->totalLocals;

    userfunc_table.push_back(f);

    funcenter_jump.insert(pair<string, unsigned>(f.id, nextinstructionlabel()));
}
