...
unsigned userfunc_newused (SymbolTableEntry* sym) {
    for (int i=0; i<userfunc_table.size(); i++) {
        ...
        if (sym->value.funVal->iaddress == userfunc_table.at(i).address)
            return i;
    }
...
void make_operand (expr* e, vmarg* arg) {
    ...
    switch (e->type)
    {
        ...
        case programfunc_e : {
            arg->type = userfunc_a;
            arg->val  = e->sym->value.funVal->iaddress;
            break;
        }
...
void generate_TABLEGETELEM (quad* q)     { generate(tablesetelem_v, q); }
...
void generate_RETURN (quad* q) {
    ...
    //todo top/append funstack
    return_list.push_back(nextinstructionlabel());
...
void generate_FUNCEND (quad* q) {
    ...
    instructions.at(return_list.back()).result.val = nextinstructionlabel();
...
void run_generate (void) {
    ...
    for (int i=0; i<funcexit_labels.size(); i++){
        cout << funcexit_labels.at(i) << endl;
    }
}
...
void print_instructions(){
    ...
    for(int i=0; i<instructions.size(); i++){
        ...
        //cout<<result_type<<"\t\t"<<arg1_type<<"("<<instructions.at(i).arg1.val<<")"<<"\t"<<arg2_type<<"("<<instructions.at(i).arg2.val<<")";
        cout << instructions.at(i).result.val;
        cout << endl;
    }
}
