...
#define currInstruction instructions.size()+1;
...
void patch_incomplete_jumps(){
    for(int i=0;i<incomplete_jumps.size();i++){
        if(incomplete_jumps.at(i).iaddress==quads.size()-1){
            cout<<instructions.size()<<" "<<incomplete_jumps.at(i).instrNo<<endl;
            instructions.at(incomplete_jumps.at(i).instrNo).result.val=instructions.size()-1;
        }
        else{
            cout<<"i am in"<<instructions.size()<<" "<<incomplete_jumps.at(i).instrNo<<endl;
